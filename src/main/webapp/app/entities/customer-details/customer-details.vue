<template>
  <div>
    <h2 id="page-heading" data-cy="CustomerDetailsHeading">
      <span v-text="$t('posdevicerestapiApp.customerDetails.home.title')" id="customer-details-heading">Customer Details</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('posdevicerestapiApp.customerDetails.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'CustomerDetailsCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('posdevicerestapiApp.customerDetails.home.createLabel')"> Create a new Customer Details </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && customerDetails && customerDetails.length === 0">
      <span v-text="$t('posdevicerestapiApp.customerDetails.home.notFound')">No customerDetails found</span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="customerDetails">
        <thead>
          <tr>
            ...
            <th scope="row" v-on:click="changeOrder('gender')">
              <span v-text="$t('posdevicerestapiApp.customerDetails.gender')">Gender</span>
            ...
            <th scope="row" v-on:click="changeOrder('phone')">
              <span v-text="$t('posdevicerestapiApp.customerDetails.phone')">Phone</span>
            ...
            <th scope="row" v-on:click="changeOrder('addressLine1')">
              <span v-text="$t('posdevicerestapiApp.customerDetails.addressLine1')">Address Line 1</span>
            ...
            <th scope="row" v-on:click="changeOrder('addressLine2')">
              <span v-text="$t('posdevicerestapiApp.customerDetails.addressLine2')">Address Line 2</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'addressLine2'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('branch')">
              <span v-text="$t('posdevicerestapiApp.customerDetails.branch')">Branch</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'branch'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('country')">
              <span v-text="$t('posdevicerestapiApp.customerDetails.country')">Country</span>
            ...
            <th scope="row" v-on:click="changeOrder('user.login')">
              <span v-text="$t('posdevicerestapiApp.customerDetails.user')">User</span>
        ...
        <tbody>
          <tr v-for="customerDetails in customerDetails" :key="customerDetails.id" data-cy="entityTable">
            ...
            <td v-text="$t('posdevicerestapiApp.Gender.' + customerDetails.gender)">{{ customerDetails.gender }}</td>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      ...
      <div class="modal-body">
        <p id="jhi-delete-customerDetails-heading" v-text="$t('posdevicerestapiApp.customerDetails.delete.question', { id: removeId })">
