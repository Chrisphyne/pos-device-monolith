<template>
  <div>
    <h2 id="page-heading" data-cy="ShoppingCartHeading">
      <span v-text="$t('posdevicerestapiApp.shoppingCart.home.title')" id="shopping-cart-heading">Shopping Carts</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('posdevicerestapiApp.shoppingCart.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'ShoppingCartCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('posdevicerestapiApp.shoppingCart.home.createLabel')"> Create a new Shopping Cart </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && shoppingCarts && shoppingCarts.length === 0">
      <span v-text="$t('posdevicerestapiApp.shoppingCart.home.notFound')">No shoppingCarts found</span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="shoppingCarts">
        <thead>
          <tr>
            ...
            <th scope="row"><span v-text="$t('posdevicerestapiApp.shoppingCart.placedDate')">Placed Date</span></th>
            <th scope="row"><span v-text="$t('posdevicerestapiApp.shoppingCart.status')">Status</span></th>
            <th scope="row"><span v-text="$t('posdevicerestapiApp.shoppingCart.totalPrice')">Total Price</span></th>
            <th scope="row"><span v-text="$t('posdevicerestapiApp.shoppingCart.paymentMethod')">Payment Method</span></th>
            <th scope="row"><span v-text="$t('posdevicerestapiApp.shoppingCart.paymentReference')">Payment Reference</span></th>
            <th scope="row"><span v-text="$t('posdevicerestapiApp.shoppingCart.customerDetails')">Customer Details</span></th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="shoppingCart in shoppingCarts" :key="shoppingCart.id" data-cy="entityTable">
            ...
            <td v-text="$t('posdevicerestapiApp.OrderStatus.' + shoppingCart.status)">{{ shoppingCart.status }}</td>
            <td>{{ shoppingCart.totalPrice }}</td>
            <td v-text="$t('posdevicerestapiApp.PaymentMethod.' + shoppingCart.paymentMethod)">{{ shoppingCart.paymentMethod }}</td>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      ...
      <div class="modal-body">
        <p id="jhi-delete-shoppingCart-heading" v-text="$t('posdevicerestapiApp.shoppingCart.delete.question', { id: removeId })">
